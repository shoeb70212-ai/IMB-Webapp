...
export default function NewLotWizard({ onNavigate }: NewLotWizardProps) {
  ...
  const [settings, setSettings] = useState<SystemSettings>(() => {
    ...
    return {
      business_name: "Kisan Trading Co.", owner_name: "Owner", phone: "", address: "",
      default_commission_percent: 6, default_labour_per_crate: 15, default_weighing_per_crate: 5,
      fruit_types: ["Mango", "Banana", "Apple", "Grapes", "Other"],
  ...
  return (
    <div className="flex-grow overflow-y-auto px-3.5 py-4 lg:p-6 pb-28 lg:pb-6 space-y-4 lg:space-y-6 animate-fade-in bg-slate-950 text-slate-200">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <button 
            ...
            className="px-4 py-2 border border-slate-800 text-xs font-bold text-slate-400 hover:text-white bg-slate-900 hover:bg-slate-850 rounded-xl cursor-pointer transition duration-150"
          >
            Discard Draft
          </button>
        </div>
      ...
      {/* Progress Line */}
      <div className="flex items-center gap-1.5 w-full bg-slate-900/50 p-2 border border-slate-850 rounded-2xl">
        {[1, 2, 3, 4].map(s => (
          <div key={s} className="flex-1 flex items-center gap-2">
            ...
            <div className={`h-1 flex-1 rounded-full ${
              step > s ? 'bg-emerald-500/25' : 'bg-slate-950'
            }`} />
          </div>
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 lg:p-6 max-w-xl mx-auto space-y-5 lg:space-y-6 shadow-xl">
          ...
          <div className="space-y-4">
            ...
            {/* Arrival Date */}
            <div>
              <label className="text-xs text-slate-400 font-semibold block mb-1">Arrival Date & Time</label>
            ...
            {/* Labor Crew */}
            <div>
              <label className="text-xs text-slate-400 font-semibold block mb-1">Allocate Loader Gang (Unloading Crew)</label>
              <select
                value={labourId}
          ...
          <div className="flex justify-end pt-4 border-t border-slate-850">
            <button
              onClick={nextStep1}
              className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl flex items-center gap-1.5 shadow-lg shadow-blue-500/10 cursor-pointer"
            >
              <span>Next: Crate Ledger</span>
      ...
      {step === 2 && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:gap-6">
          ...
          <div className="glass-panel rounded-3xl p-4 lg:p-6 h-fit space-y-4 lg:space-y-5 shadow-xl">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Fruit</label>
                  <select
                    value={nc.fruit}
                ...
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Quality Grade</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Crate Count</label>
                  <input
                    type="number"
                    min="1"
                    value={nc.qty}
                    onChange={(e) => setNc(prev => ({ ...prev, qty: parseInt(e.target.value) || 0 }))}
                    className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-200 focus:outline-none font-mono"
                  />
                </div>
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Base Price / kg (₹)</label>
                  <input
                    type="number"
                    min="0"
                    value={nc.rate}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Gross Wt / Cr (kg)</label>
                  <input
                    ref={grossInputRef}
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Tare Box Wt (kg)</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-blue-500/10 transition duration-150 text-sm animate-pulse-slow"
            >
          ...
          <div className="lg:col-span-2 glass-panel rounded-3xl p-4 lg:p-6 shadow-xl flex flex-col justify-between min-h-[380px]">
            <div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-slate-800 text-slate-500 uppercase font-semibold">
                        <th className="py-2.5 px-3">Fruit / Grade</th>
                        <th className="py-2.5 px-3 text-center">Crates</th>
                        <th className="py-2.5 px-3 text-right">Gross/Cr</th>
                        <th className="py-2.5 px-3 text-right">Net Wt. Total</th>
                        <th className="py-2.5 px-3 text-right">Auction Rate</th>
                        <th className="py-2.5 px-3 text-right">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-850/50">
                      {items.map(item => (
                        <tr key={item.id} className="hover:bg-slate-800/10 transition duration-100">
                          <td className="py-2.5 px-3 font-semibold text-white">
                            {item.fruit} <span className="text-[10px] text-blue-400 font-bold ml-1 px-1.5 py-0.5 rounded bg-blue-500/10 border border-blue-500/10">{item.grade}</span>
                          </td>
                          <td className="py-2.5 px-3 text-center font-mono text-slate-300">{item.qty}</td>
                          <td className="py-2.5 px-3 text-right font-mono text-slate-400">{item.gross} kg</td>
                          <td className="py-2.5 px-3 text-right font-mono text-slate-300">{item.net_total.toFixed(1)} kg</td>
                          <td className="py-2.5 px-3 text-right font-bold text-emerald-400 font-mono">₹{item.rate}/kg</td>
                          <td className="py-2.5 px-3 text-center">
                            <button
                              onClick={() => deleteCrateItem(item.id)}
                              className="p-1 text-slate-500 hover:text-rose-400 rounded-lg hover:bg-slate-800 cursor-pointer transition"
                            >
              ...
              <div className="md:hidden space-y-3 pr-1">
                {items.length === 0 ? (
                  ...
                  items.map(item => (
                    <div key={item.id} className="p-3 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col gap-1.5">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-white text-xs">{item.fruit}</span>
                          <span className="px-1.5 py-0.5 bg-blue-500/10 text-blue-400 rounded text-[9px] uppercase tracking-wide font-sans font-bold">{item.grade}</span>
                        </div>
                        <button
                          onClick={() => deleteCrateItem(item.id)}
                          className="p-1 text-slate-500 hover:text-rose-455 rounded-lg hover:bg-slate-800 transition"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-800 mt-4">
              <button
                onClick={() => setStep(1)}
                className="px-4 py-2 border border-slate-850 text-xs font-bold text-slate-400 hover:text-white bg-slate-850 hover:bg-slate-800 rounded-xl flex items-center gap-1 cursor-pointer transition"
              >
              ...
              <button
                onClick={nextStep2}
                className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-xl flex items-center gap-1 cursor-pointer shadow-lg shadow-blue-500/10 transition"
              >
                <span>Next: Crate Distribution</span>
                <ArrowRight className="w-4 h-4" />
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="glass-panel rounded-3xl p-4 lg:p-6 h-fit space-y-5 shadow-xl">
            ...
            <div className="space-y-4">
              {/* Target item */}
              <div>
                <label className="text-[11px] text-slate-400 font-semibold block mb-1">Select Lot Grade</label>
                <select
              ...
              {/* Select Buyer */}
              <div>
                <label className="text-[11px] text-slate-400 font-semibold block mb-1">Assign to Buyer</label>
                <select
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Crates Allocated</label>
                  <input
                    type="number"
                    min="1"
                    value={allocForm.qty}
                ...
                <div>
                  <label className="text-[11px] text-slate-400 font-semibold block mb-1">Final Bid / kg (₹)</label>
                  <input
                    type="number"
                    min="0"
              ...
              {/* Payment Mode */}
              <div>
                <label className="text-[11px] text-slate-400 font-semibold block mb-1.5">Payment Terms</label>
                <div className="grid grid-cols-4 gap-1.5 bg-slate-950 p-1 border border-slate-800 rounded-xl">
                  {(['cash', 'credit', 'upi', 'bank'] as PaymentMode[]).map(mode => (
                    <button
                      ...
                      className={`py-1 rounded-md text-[10px] font-bold uppercase tracking-wider cursor-pointer transition ${
                        allocForm.mode === mode
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-bold rounded-xl flex items-center justify-center gap-1.5 cursor-pointer shadow-lg shadow-blue-500/10 transition duration-150 text-sm"
            >
              <Plus className="w-4 h-4" />
            ...
            <div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-850/50">
                      {allocations.map((a, idx) => (
                        <tr key={a.id} className="hover:bg-slate-800/10 transition duration-100">
                          <td className="py-2.5 px-3 font-semibold text-white">
                            {a.fruit} <span className="text-[9px] text-blue-400 font-bold ml-1">{a.grade}</span>
                          </td>
                          <td className="py-2.5 px-3 font-medium text-slate-300 truncate max-w-[100px]">{a.buyer_name}</td>
                          <td className="py-2.5 px-3 text-center font-mono text-slate-300">{a.qty}</td>
                          <td className="py-2.5 px-3 text-right font-mono text-slate-400">{a.net_weight.toFixed(1)} kg</td>
                          <td className="py-2.5 px-3 text-right font-mono text-slate-400">₹{a.rate}/kg</td>
                          <td className="py-2.5 px-3 text-right font-bold text-emerald-400 font-mono">₹{a.amount.toFixed(0)}</td>
                          <td className="py-2.5 px-3 text-center">
              ...
              <div className="md:hidden space-y-3 pr-1">
                {allocations.length === 0 ? (
                  ...
                  allocations.map((a, idx) => (
                    <div key={a.id} className="p-3 bg-slate-950/40 border border-slate-850 rounded-2xl flex flex-col gap-2">
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-white text-xs">{a.fruit}</span>
                          <span className="px-1.5 py-0.5 bg-blue-500/10 text-blue-400 rounded text-[9px] uppercase tracking-wide font-sans font-bold">{a.grade}</span>
                        </div>
                      ...
                      <div className="flex justify-between items-end text-xs">
                        <div className="text-[11px]">
                          <div className="text-slate-350">Buyer: <span className="font-semibold text-white">{a.buyer_name}</span></div>
                          <div className="text-[10px] text-slate-505 font-mono mt-0.5">Qty: {a.qty} Cr. ({a.net_weight.toFixed(1)}kg) @ ₹{a.rate}/kg</div>
                        </div>
                        <div className="text-right">
                          <span className="font-bold text-emerald-450 font-mono text-xs">₹{a.amount.toFixed(0)}</span>
                        </div>
                      </div>
                    </div>
                  ))
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 lg:p-6 shadow-xl space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-slate-800/80 pt-3 space-y-3">
                ...
                {summary.charges.map((ch, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm">
                    <span className="text-slate-400 text-xs">{ch.desc}</span>
                    <div className="flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        className="w-18 px-2 py-1 bg-slate-950 border border-slate-800 rounded text-right text-xs font-mono text-white focus:outline-none focus:border-blue-600"
                      />
                    </div>
                  </div>
                ))}
              ...
              <div className="border-t border-slate-800 pt-3 flex justify-between items-center">
                <span className="text-sm font-bold text-white">Net Payable to Seller</span>
                ...
                summary.buyer_charges.map((ch, idx) => (
                  <div key={idx} className="flex justify-between items-center p-3 bg-slate-950 rounded-xl border border-slate-850">
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 mt-0.5">{ch.desc} ({ch.qty} Cr.)</p>
                    </div>
                    <div className="flex items-center gap-1">
                      ...
                      <input
                        ...
                        className="w-18 px-2 py-1 bg-slate-900 border border-slate-800 rounded text-right text-xs font-mono text-white focus:outline-none focus:border-blue-600"
                      />
                    </div>
                  </div>
                ))
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 lg:p-6 shadow-xl flex flex-col justify-between h-full min-h-[280px]">
            <div className="space-y-4">
              <div className="flex items-center gap-2 text-rose-400">
                ...
                <h3 className="text-sm font-bold font-display uppercase tracking-wider">Final review</h3>
              </div>
              ...
              <div className="p-3.5 bg-slate-950 border border-slate-850 rounded-2xl space-y-1 text-xs">
                <div className="flex justify-between text-slate-400">
                  <span>Total Crates Count:</span>
                  <span className="font-mono font-bold text-white">{items.reduce((s, i) => s + i.qty, 0)}</span>
                </div>
                <div className="flex justify-between text-slate-400">
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-800 mt-4">
              <button
                onClick={() => setStep(3)}
                className="px-4 py-2 border border-slate-850 text-xs font-bold text-slate-400 hover:text-white bg-slate-850 hover:bg-slate-800 rounded-lg flex items-center gap-1 cursor-pointer"
              >
                <ArrowLeft className="w-4 h-4" />
                <span>Back</span>
              </button>
              <button
                ...
                className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 cursor-pointer shadow-lg shadow-emerald-500/10"
              >
